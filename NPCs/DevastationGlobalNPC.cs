...
namespace Azercadmium.NPCs
{
    public class DevastationGlobalNPC : GlobalNPC
    {
            ...
            if (AzercadmiumWorld.devastation)
            {
                switch (npc.type)
                {
                    ...
                    switch (npc.netID)
                    {
                        ...
                        {
                            target.GetModPlayer<AzercadmiumPlayer>().GiveSlimeColor(npc.color);
                            target.AddBuff(ModContent.BuffType<Buffs.Devastation.SlimyOoze>(), Main.rand.Next(2, 6) * 60, true);
                        }
                        break;
                        case NPCID.BlueSlime:
                        {
                    ...
                    case NPCID.KingSlime:
                    {
                        target.GetModPlayer<AzercadmiumPlayer>().GiveSlimeColor(new Color(0, 80, 255, 100));
                        target.AddBuff(ModContent.BuffType<Buffs.Devastation.SlimyOoze>(), Main.rand.Next(2, 6) * 60, true);
                    }
                    break;
                    case NPCID.EyeofCthulhu:
        ...
        private int AITimer;
        private int AITimer2;
        private int AITimer3;
        private readonly int AITimer4;
        private bool AIFlag;
        private bool AIFlag2;
