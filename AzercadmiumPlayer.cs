...
namespace Azercadmium
{
    public class AzercadmiumPlayer : ModPlayer
    {
        ...
        private Color? playerSlimeColor;

        private Color playerSlimeColorValue;

        private float slimeColorR;

        private float slimeColorG;

        private float slimeColorB;

        /// <summary>
        /// 
        /// </summary>
        /// <param name="color">The color of the slime</param>
        /// <param name="disco">Unused.</param>
        public void GiveSlimeColor(Color color, bool disco = false)
        {
            if (playerSlimeColor == null)
            {
                playerSlimeColor = color;
                playerSlimeColorValue = color;
            }
            else
            {
                Color c = Color.Lerp(playerSlimeColorValue, color, 0.5f);
                playerSlimeColor = c;
                playerSlimeColorValue = c;
            }
            slimeColorR = playerSlimeColorValue.R / 255f;
            slimeColorG = playerSlimeColorValue.G / 255f;
            slimeColorB = playerSlimeColorValue.B / 255f;
        }

        public void ClearSlimeColor()
        {
            playerSlimeColor = null;
            playerSlimeColorValue = default(Color);
            slimeColorR = 0f;
            slimeColorG = 0f;
            slimeColorB = 0f;
        }
    ...
    public override void ResetEffects()
        ...
        public override void DrawEffects(PlayerDrawInfo drawInfo, ref float r, ref float g, ref float b, ref float a, ref bool fullBright)
        {
            ...
            if (slimyOoze)
            {
                if (Main.rand.NextBool(4) && drawInfo.shadow == 0f)
                {
                    int dust = Dust.NewDust(drawInfo.position - new Vector2(2f, 2f), player.width + 4, player.height + 4, ModContent.DustType<Dusts.Devastation.Debuffs.SlimyOozeDust>(), player.velocity.X * 0.4f, player.velocity.Y * 0.4f);
                    Main.dust[dust].noGravity = true;
                    Main.dust[dust].velocity *= 1.8f;
                    Main.dust[dust].velocity.Y -= 0.5f;
                    Main.dust[dust].color = playerSlimeColorValue;
                    Main.playerDrawDust.Add(dust);
                }
                r *= slimeColorR;
                g *= slimeColorG;
