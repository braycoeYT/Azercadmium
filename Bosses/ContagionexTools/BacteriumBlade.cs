...
namespace Azercadmium.NPCs.Bosses.ContagionexTools
{
	[AutoloadBossHead]
	public class BacteriumBlade : ModNPC
	{
        ...
        public override void AI() {
			...
			if (Timer > 600)
			npc.dontTakeDamage = !Main.player[npc.target].GetModPlayer<AzercadmiumPlayer>().ZoneMicrobiome;
			else
			...
			{
				if (Timer < 480)
				npc.position = new Vector2(target.position.X, target.position.Y - 300);
				if (Timer > 60 && npc.alpha != 0)
					npc.alpha--;
			}
			//attack
			else
			{
				if (npc.ai[0] == 0f)
				{
					...
					{
						npc.velocity.X = num321 * 8; //speed?
						npc.velocity.Y = num322 * 8; //speed?
					}
					else if (Vector2.Distance(npc.Center, target.Center) > 1000 || npc.life < npc.lifeMax / 2)
					{
						npc.velocity.X = num321 * 4; //speed?
						npc.velocity.Y = num322 * 4; //speed?
					}
					else
					{
						npc.velocity.X = num321 * 2; //speed?
						npc.velocity.Y = num322 * 2; //speed?
					}
				...
				if (npc.ai[0] == 1f)
				{
					...
					if (npc.ai[1] == 0)
					{
						...
						{
							rotation += 45;
							Projectile.NewProjectile(npc.Center, new Vector2(0, 15).RotatedBy((Math.PI / 180) * rotation, default), mod.ProjectileType("TocContagionOrbHostile"), 70, 1f, Main.myPlayer);
						}
					}
					npc.velocity *= 0.99f;
					...
					else
					{
						...
						if (npc.ai[1] % 30 == 0)
						Projectile.NewProjectile(npc.Center, new Vector2(0, 15).RotatedBy((Math.PI / 180) * Main.rand.Next(-180, 180), default), mod.ProjectileType("TocBacteriumBladeFake"), 90, 1f, Main.myPlayer);
						npc.velocity *= 0.96f;
						npc.ai[1] += 1f;
