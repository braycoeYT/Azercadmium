...
{
	public class Caelum1 : ModProjectile
	{
		public override void SetStaticDefaults() {
			DisplayName.SetDefault("Caelum I");
		...
		public override void AI() {
			...
			if (foundTarget) {
				Timer++;
				if (Timer % 120 == 0)
				Projectile.NewProjectile(projectile.Center, projDir, mod.ProjectileType("CaelumBoltSmall"), projectile.damage, projectile.knockBack, Main.myPlayer);
			}
		}
		public override void PostAI() {
			if (Main.rand.NextBool()) {
				Dust dust = Dust.NewDustDirect(projectile.position, projectile.width, projectile.height, mod.DustType("CaelumDust"));
				dust.noGravity = true;
				dust.scale = 1f;
			}
		}
