...
namespace Azercadmium.Projectiles.Jungle
{
	...
	{
		private const string ChainTexturePath = "Azercadmium/Projectiles/Jungle/SporophyteChain";
		...
		public override void OnHitNPC(NPC target, int damage, float knockback, bool crit) {
			target.AddBuff(BuffID.Poisoned, 60*Main.rand.Next(6, 13));
		}
		...
		public override void AI() {
			...
			if (Timer % 20 == 0) {
				...
				Projectile.NewProjectile(projectile.Center, new Vector2(0, -5).RotatedByRandom(1f), mod.ProjectileType("StingerPassive"), (int)(projectile.damage * 0.5f), projectile.knockBack / 3, Main.myPlayer);
			}

			var dust = Dust.NewDustDirect(projectile.position, projectile.width, projectile.height, 44, projectile.velocity.X * 0.4f, projectile.velocity.Y * 0.4f, 100, default, 1.5f);
			dust.scale /= 2;
			dust.noGravity = true;
			dust.velocity /= 2f;
			...
			if (projectile.ai[0] == 0f) {
				...
				float elasticFactorB = 1.2f / player.meleeSpeed;
				float maxStretchLength = 320f; // This is the furthest the flail can stretch before being forced to retract. Make sure that this is a bit less than maxChainLength so you don't accidentally reach maxStretchLength on the initial throw.
