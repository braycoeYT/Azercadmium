...
{
	public class ShadowMoon : ModProjectile
	{
		private const string ChainTexturePath = "Azercadmium/Projectiles/Jelly/ShadowMoonChain";
		...
		public override void AI() {
			var dust = Dust.NewDustDirect(projectile.position, projectile.width, projectile.height, ModContent.DustType<Dusts.Jelly.JellyDust>(), projectile.velocity.X * 0.4f, projectile.velocity.Y * 0.4f, 100, default, 1.5f);
			dust.scale /= 2;
			dust.noGravity = true;
			dust.velocity /= 2f;
			...
			if (projectile.ai[0] == 0f) {
				...
				if (!bool1) {
					for (int i = 0; i < 9; i++) {
						Projectile.NewProjectile(projectile.Center, new Vector2(0, 10).RotatedBy(MathHelper.ToRadians(i*40)), ModContent.ProjectileType<EerieSpearProj>(), (int)(projectile.damage * 0.33f), 1.2f, Main.myPlayer);
					}
					bool1 = true;
				}
				...
				float maxStretchLength = 270f; // This is the furthest the flail can stretch before being forced to retract. Make sure that this is a bit less than maxChainLength so you don't accidentally reach maxStretchLength on the initial throw.
