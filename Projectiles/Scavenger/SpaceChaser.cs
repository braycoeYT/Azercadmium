...
{
	public class SpaceChaser : ModProjectile
	{
		...
		public override void AI() {
			...
			{
				player.ClearBuff(BuffType<Buffs.Minions.SpaceChaser>());
			}
			if (player.HasBuff(BuffType<Buffs.Minions.SpaceChaser>()))
			{
            ...
            #region Projectile
			Vector2 projDir = Vector2.Normalize(targetCenter - projectile.Center) * 14;
			if (Timer % 30 == 0 && foundTarget) {
				Main.PlaySound(SoundID.Item91, projectile.position);
				Projectile.NewProjectile(projectile.Center, projDir, mod.ProjectileType("MatrixBlastFriendly"), projectile.damage, projectile.knockBack, Main.myPlayer);
			}
